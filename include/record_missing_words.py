...
def record(word, directory = args.output_dir):
    os.system('rec "' + os.path.join(directory, word) + args.ending + '.wav"')
    
def convert(word, directory = "./"):
    wavpath = os.path.join(directory, word) +  args.ending + '.wav'
    mp3path = os.path.join(directory, word.lower()) +  args.ending + '.mp3'
...
for filename in allfilenames:
    ...
    if args.format == "mp3":
        ...
        for word in missing_words:
            remove_wave(word + args.ending + ".wav", soundsdir)
